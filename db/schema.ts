...
export const portfolioPublications = sqliteTable(
  "portfolio_publications",
  {
    runId: text("run_id").primaryKey().references(() => quantRuns.id, { onDelete: "cascade" }),
    status: text("status", { enum: ["PENDING", "ACTIVE", "REJECTED"] }).notNull().default("PENDING"),
    methodologyVersion: text("methodology_version").notNull(),
    expectedAllocations: integer("expected_allocations").notNull(),
    receivedAllocations: integer("received_allocations").notNull().default(0),
    allocationPayloadHash: text("allocation_payload_hash").notNull(),
    summaryHash: text("summary_hash").notNull(),
    summaryReceived: integer("summary_received", { mode: "boolean" }).notNull().default(false),
    regimeLabel: text("regime_label"),
    maxEquityExposure: real("max_equity_exposure"),
    minimumCashAllocation: real("minimum_cash_allocation"),
    maxPortfolioRisk: real("max_portfolio_risk"),
    positionCap: real("position_cap").notNull(),
    sectorCap: real("sector_cap").notNull(),
    correlationThreshold: real("correlation_threshold").notNull(),
    positionCount: integer("position_count"),
    capitalDeployed: real("capital_deployed"),
    cashAllocation: real("cash_allocation"),
    portfolioRisk: real("portfolio_risk"),
    largestPosition: real("largest_position"),
    top5Concentration: real("top5_concentration"),
    portfolioQuantScore: real("portfolio_quant_score"),
    portfolioBeta: real("portfolio_beta"),
    expectedVolatility: real("expected_volatility"),
    sectorExposureJson: text("sector_exposure_json"),
    correlationClustersJson: text("correlation_clusters_json"),
    automaticExecution: integer("automatic_execution", { mode: "boolean" }).notNull().default(false),
    startedAt: text("started_at").notNull(),
    committedAt: text("committed_at"),
  },
  (table) => [index("portfolio_publications_status_idx").on(table.status)],
);

export const portfolioAllocations = sqliteTable(
  "portfolio_allocations",
  {
    ...
    symbol: text("symbol").notNull().references(() => instruments.symbol),
    tradeId: text("trade_id").notNull(),
    tradeState: text("trade_state").notNull(),
    sector: text("sector").notNull(),
    quantScore: real("quant_score").notNull(),
    lastClose: real("last_close").notNull(),
    atrPct: real("atr_pct").notNull(),
    stopDistancePct: real("stop_distance_pct").notNull(),
    averageTradedValue20: real("average_traded_value_20").notNull(),
    scoreMultiplier: real("score_multiplier").notNull(),
    volatilityMultiplier: real("volatility_multiplier").notNull(),
    liquidityMultiplier: real("liquidity_multiplier").notNull(),
    correlationMultiplier: real("correlation_multiplier").notNull(),
    targetWeight: real("target_weight").notNull(),
    riskBudget: real("risk_budget").notNull(),
    riskContribution: real("risk_contribution").notNull(),
    volatilityContribution: real("volatility_contribution"),
    beta: real("beta"),
    correlationCluster: text("correlation_cluster").notNull(),
    sectorCapApplied: integer("sector_cap_applied", { mode: "boolean" }).notNull(),
  ...
  (table) => [
    primaryKey({ columns: [table.runId, table.symbol] }),
    index("portfolio_allocations_run_risk_idx").on(table.runId, table.riskContribution),
    index("portfolio_allocations_run_sector_idx").on(table.runId, table.sector),
  ],
);
